...
local function mkDefault()return{esp_on=false,esp_names=false,esp_lines=false,esp_key="T",esp_char_r=100,esp_char_g=220,esp_char_b=100,esp_avatar=true,esp_rainbow=false,hbx_on=false,hbx_size=5,hbx_show=false,hbx_show2=false,hbx_key="G",hbx_vis_check=true,stream_mode=false,summer_on=false,panel_bg=true,notifs=true,lang="English",gui_key="L",Whitelist={}}end
local S=mkDefault()
pcall(function()local d=HttpService:JSONDecode(readfile(CONFIG_FILE))for k,v in pairs(S)do if d[k]~=nil then S[k]=d[k]end end end)
local function save()pcall(function()writefile(CONFIG_FILE,HttpService:JSONEncode(S))end)end

local wlSet={}
local function rebuildWlSet()wlSet={}for _,n in ipairs(S.Whitelist)do if type(n)=="string"and n~=""then wlSet[n:lower()]=true end end end
rebuildWlSet()
local function isWhitelisted(p)if not p or not p.Parent then return false end return wlSet[p.Name:lower()]==true end
local function addWhitelist(name)if name==""then return false end local nl=name:lower()if wlSet[nl]then return false end table.insert(S.Whitelist,name)wlSet[nl]=true save()return true end
local function removeWhitelist(name)local nl=name:lower()if not wlSet[nl]then return false end for i,n in ipairs(S.Whitelist)do if n:lower()==nl then table.remove(S.Whitelist,i)break end end wlSet[nl]=nil save()return true end
local function shouldSkipPlayer(p)if not p or not p.Parent then return true end if p==player then return true end if isWhitelisted(p)then return true end return false end

local Locale={English={tab_esp="ESP",tab_hbx="HBX",tab_trg="TRG",tab_cfg="CFG",esp_on="Enable ESP",esp_on_d="Show enemies through walls.",esp_names="Show Enemy Name",esp_lines="ESP Lines",esp_lines_d="Draw lines from your character to enemy positions.",esp_key="ESP Keybind",esp_color="ESP Color",esp_color_d="Customize the character highlight color.",esp_avatar="Show Avatar",esp_avatar_d="Show player avatar thumbnail above ESP.",esp_rainbow="Rainbow Aura",esp_rainbow_d="Cycle through all colors automatically.",hbx_on="Enable Hitbox",hbx_on_d="Show and expand enemy hitboxes.",hbx_size="Hitbox Size",hbx_show="Show Hitbox",hbx_show2="Show Hitbox (Always)",hbx_show2_d="Shows the hitbox box at the selected size regardless of whether the player is hidden.",hbx_key="Hitbox Keybind",hbx_vis="Visible Check",hbx_vis_d="Only register hits when the enemy is actually visible. Prevents kills through walls.",summer_on="Summer 2026",summer_on_d="Collects Summer 2026 drops automatically. Only in matches.",st_bg="Toggle Panel Background",st_notif="Enable Notifications",st_lang="Language",st_key="Toggle UI",st_stream="Stream Mode",st_stream_d="Hides GUI and disables all ESP visuals. Hotkey: RightAlt.",st_r1="Reset Toggle UI Keybind",st_r1_d="Reset this keybind to its default value.",st_r2="Reset All Keybinds",st_r2_d="Reset all keybinds to their default values.",n_on="Enabled",n_off="Disabled",n_reset="Keybind reset",},Español={tab_esp="ESP",tab_hbx="HBX",tab_trg="TRG",tab_cfg="CFG",esp_on="Activar ESP",esp_on_d="Muestra enemigos a través de paredes.",esp_names="Nombre del Enemigo",esp_lines="Líneas ESP",esp_lines_d="Dibuja líneas desde tu personaje hasta los enemigos.",esp_key="Tecla ESP",esp_color="Color ESP",esp_color_d="Personaliza el color del resaltado del personaje.",esp_avatar="Mostrar Avatar",esp_avatar_d="Muestra el avatar del jugador sobre el ESP.",esp_rainbow="Aura Arcoíris",esp_rainbow_d="Cicla automáticamente por todos los colores.",hbx_on="Activar Hitbox",hbx_on_d="Muestra y expande las hitboxes enemigas.",hbx_size="Tamaño Hitbox",hbx_show="Mostrar Hitbox",hbx_show2="Mostrar Hitbox (Siempre)",hbx_show2_d="Muestra la caja de hitbox al tamaño seleccionado sin importar si el jugador está oculto.",hbx_key="Tecla Hitbox",hbx_vis="Visible Check",hbx_vis_d="Solo registra el hit si el enemigo está a la vista. Evita matar a través de paredes.",summer_on="Summer 2026",summer_on_d="Recolecta los drops del Summer 2026 automáticamente. Solo en partidas.",st_bg="Fondo del Panel",st_notif="Activar Notificaciones",st_lang="Idioma",st_key="Alternar UI",st_stream="Stream Mode",st_stream_d="Oculta la GUI y desactiva el ESP visual. Tecla: RightAlt.",st_r1="Restablecer Tecla UI",st_r1_d="Restablece esta tecla a su valor predeterminado.",st_r2="Restablecer Todas",st_r2_d="Restablece todas las teclas a sus valores predeterminados.",n_on="Activado",n_off="Desactivado",n_reset="Tecla restablecida",}}
local L=function(k)local tbl=Locale[S.lang]or Locale.English return tbl[k]or(Locale.English[k]or k)end

local C={BG=Color3.fromRGB(5,5,7),CARD=Color3.fromRGB(14,12,20),HEADER=Color3.fromRGB(8,6,13),TABBAR=Color3.fromRGB(6,5,9),TEXT=Color3.fromRGB(212,202,228),DIM=Color3.fromRGB(102,92,120),ACCENT=Color3.fromRGB(141,122,174),ACCENT2=Color3.fromRGB(170,148,210),DIV=Color3.fromRGB(28,24,40),TOG_ON=Color3.fromRGB(141,122,174),TOG_OFF=Color3.fromRGB(38,32,54),THUMB=Color3.fromRGB(228,220,240),KEY_BG=Color3.fromRGB(22,18,32),KEY_TXT=Color3.fromRGB(141,122,174),BORDER=Color3.fromRGB(58,50,80),RED=Color3.fromRGB(180,60,70),GREEN=Color3.fromRGB(90,180,100),}
...
pcall(function()local cg=game:GetService("CoreGui"):FindFirstChild("x7sV1")if cg then cg:Destroy()end end)
...
for i=1,14 do
    ...
    task.spawn(function()
        ...
        while dot.Parent do
            ...
            dot.BackgroundTransparency=1
            TweenService:Create(dot,TweenInfo.new(1.1,Enum.EasingStyle.Sine),{BackgroundTransparency=0.42}):Play()
            TweenService:Create(dot,TweenInfo.new(3.2+math.random(),Enum.EasingStyle.Sine,Enum.EasingDirection.Out),{Position=UDim2.fromOffset(startX+math.random(-16,16),math.max(40,startY-math.random(24,58)))}):Play()
            task.wait(1.4)
            TweenService:Create(dot,TweenInfo.new(1.1,Enum.EasingStyle.Sine),{BackgroundTransparency=1}):Play()
            task.wait(2.2+math.random())
        end
    end)
end

TweenService:Create(glow,TweenInfo.new(6,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{BackgroundTransparency=0.88}):Play()
panel.Size=UDim2.fromOffset(GW-34,GH-34)
panel.BackgroundTransparency=1
TweenService:Create(panel,TweenInfo.new(0.45,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.fromOffset(GW,GH),BackgroundTransparency=0}):Play()
...
headerGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(4,3,4)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(13,10,13)),ColorSequenceKeypoint.new(1,Color3.fromRGB(4,3,4))})
...
sbGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(4,3,4)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(8,5,8)),ColorSequenceKeypoint.new(1,Color3.fromRGB(4,3,4))})
sbGrad.Rotation=90
...
TweenService:Create(logoGlow,TweenInfo.new(3,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.25,Thickness=2}):Play()
...
TweenService:Create(dotsDeco,TweenInfo.new(2.5,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{TextTransparency=0.35}):Play()
...
local function makeSBChain(parent,y)
    ...
    cg.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(0,0,0)),ColorSequenceKeypoint.new(0.2,Color3.fromRGB(58,58,58)),ColorSequenceKeypoint.new(0.5,accentColor),ColorSequenceKeypoint.new(0.8,Color3.fromRGB(58,58,58)),ColorSequenceKeypoint.new(1,Color3.fromRGB(0,0,0))})
...
local NAV_DATA={{icon="⌂",label="Inicio"},{icon="⚙",label="Ajustes"}}
...
local function applyAccent(newCol)
    ...
    panelStroke.Color=Color3.fromRGB(58,58,58)
    ...
    for _,ch in ipairs(sidebar:GetChildren())do
        if ch:IsA("Frame")and ch:FindFirstChildOfClass("UIGradient")then
            ...
            if g and #g.Color.Keypoints>=3 then
                g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(0,0,0)),ColorSequenceKeypoint.new(0.2,Color3.fromRGB(58,58,58)),ColorSequenceKeypoint.new(0.5,newCol),ColorSequenceKeypoint.new(0.8,Color3.fromRGB(58,58,58)),ColorSequenceKeypoint.new(1,Color3.fromRGB(0,0,0))})
            end
        end
    end
end
...
local function makeToggle(parent,titleKey,descKey,stateKey,cb)
    ...
    local function refresh()
        ...
        TweenService:Create(thumb,TI,{Position=on and UDim2.fromOffset(TW-20,3)or UDim2.fromOffset(2,3),BackgroundColor3=on and Color3.fromRGB(239,239,239)or Color3.fromRGB(58,58,58)}):Play()
    end
...
local tabPages={pages[1],pages[1],pages[1],pages[2]}
...
local espObjects={}
local BODY_PARTS={"Head","Torso","UpperTorso","LowerTorso","LeftArm","RightArm","LeftLeg","RightLeg","LeftUpperArm","LeftLowerArm","LeftHand","RightUpperArm","RightLowerArm","RightHand","LeftUpperLeg","LeftLowerLeg","LeftFoot","RightUpperLeg","RightLowerLeg","RightFoot","HumanoidRootPart"}
...
RunService.RenderStepped:Connect(function()
    ...
    for p,obj in pairs(espObjects)do
        ...
        if S.hbx_on and S.hbx_vis_check and _hbxOriginals[p]and _hbxOriginals[p].proxy then
            ...
            local targetSize=isVis2 and(S.hbx_size*2)or 2
            ...
            if showName then
                ...
                if nameLbl then
                    if S.hbx_vis_check then
                        ...
                        nameLbl.TextColor3=isVis3 and getEspColor()or Color3.fromRGB(220,80,80)
                    else
                ...
                if bg2 then
                    ...
                    if st then
                        if S.hbx_vis_check then
                            ...
                            st.Color=isVis4 and getEspColor()or Color3.fromRGB(220,80,80)
                        else
        ...
        if obj.selBox and typeof(obj.selBox)=="Instance"then
            if S.hbx_on and S.hbx_show and onS and _hbxOriginals[p]and _hbxOriginals[p].proxy then
                local isVis2=myChar and isVisible(root,myChar)
                local col3d=(S.hbx_vis_check and not isVis2)and Color3.fromRGB(220,80,80)or getEspColor()
...
task.spawn(function()
    ...
    while task.wait(0.3)do
        ...
        for _,spawn in ipairs(folder:GetChildren())do
            local touch=spawn:FindFirstChild("Touch")or spawn:FindFirstChildOfClass("Part")or spawn:FindFirstChildOfClass("MeshPart")or(spawn:IsA("BasePart")and spawn)
            if touch then
                pcall(function()
